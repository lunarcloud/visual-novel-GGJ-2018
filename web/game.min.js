...
// lib/game/colors.js
ig.baked=true;ig.module('game.colors').defines(function(){NormalGameColors={DialogBG:"#1F1F1F",DialogFG:"#cfcfcf"};AltGameColors={DialogBG:"#1F1F1F",DialogFG:"#C4CFA1"};GameColors=NormalGameColors;hexToRgbA=function(hex,opacity){var c;if(/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)){c=hex.substring(1).split('');if(c.length==3){c=[c[0],c[0],c[1],c[1],c[2],c[2]];}
...
{if(typeof(menu)!=="object")throw"Must provide menu object";if(menu.useTrueFont){if(typeof(menu.trueFont)==="undefined"||menu.trueFont instanceof ss.TrueFont===false)throw"menu.trueFont must be a ss.TrueFont";}else{if(typeof(menu.font)==="undefined"||menu.font instanceof ig.Font===false)throw"menu.font must be a ig.Font";}
...
{this.text=this.textGetter();if(typeof(this.text)!=="string"){console.error("Option text update failed to produce valid text.");return;}
...
ig.baked=true;ig.module('game.menu.title').requires('game.i18n','plugins.ss.menu.menu','plugins.ss.true-font').defines(function(){TitleMenu=ss.Menu.extend({startBlinkTimer:null,useTrueFont:true,trueFont:new ss.TrueFont("Cavalcade",42,"#ccc","/fonts/Cavalcade/Cavalcade-Regular.css"),useSelectedFont:true,selectedTrueFont:new ss.TrueFont("Cavalcade",42,"#fff","/fonts/Cavalcade/Cavalcade-Regular.css"),Page:{MAIN:0,SETTINGS:1,CONTROLS:2,LANGUAGE:3,LOAD:4,QUITSURE:5},actions:{prev:["up","left"],next:["down","right"],select:["primary"],back:["secondary"]},init:function()
{this.startBlinkTimer=new ig.Timer();this.createMenuOptions();this.parent();},titleFont:new ss.TrueFont("Cavalcade",42,"#ccc","/fonts/Cavalcade/Cavalcade-Regular.css"),titleText:function(){return ig.i18n().GameTitle;},pageBack:function()
...
{var menu=this;var options=[];for(var i=0;i<Object.keys(menu.Page).length;i++)options.push([]);var firstYAfterTitle=this.titleFont.textSize+40;var nextOptionSpace=function(page)
{if(options[page].length<1)return firstYAfterTitle;else if(!isNaN(options[page][options[page].length-1].endY))return options[page][options[page].length-1].endY+10;else return options[page][options[page].length-1].y+options[page][options[page].length-1].height+10;};var backButton=new ss.MenuOption(menu,function(){return ig.i18n().menuback;},function(){menu.pageBack();},8,this.titleFont.textSize,ig.Font.ALIGN.LEFT,0);options[menu.Page.MAIN].push(new ss.MenuOption(menu,function(){return ig.i18n().MainMenu.Main["Play"];},function(){ig.game.startNewGame();},(ig.system.width+10)/2,nextOptionSpace(menu.Page.MAIN),ig.Font.ALIGN.CENTER,undefined));options[menu.Page.MAIN].push(new ss.MenuOption(menu,function(){return ig.i18n().MainMenu.Main["Load"];},function(){menu.changePage(menu.Page.LOAD);},(ig.system.width+10)/2,nextOptionSpace(menu.Page.MAIN),ig.Font.ALIGN.CENTER,undefined));options[menu.Page.MAIN].push(new ss.MenuOption(menu,function(){return ig.i18n().MainMenu.Main["Options"];},function(){menu.changePage(menu.Page.SETTINGS);},(ig.system.width+10)/2,nextOptionSpace(menu.Page.MAIN),ig.Font.ALIGN.CENTER,undefined));if(ig.system.isChromeApp||ig.system.isPopup)
{options[menu.Page.MAIN].push(new ss.MenuOption(menu,function(){return ig.i18n().MainMenu.Main["Quit"];},function(){menu.changePage(menu.Page.QUITSURE);},(ig.system.width+10)/2,nextOptionSpace(menu.Page.MAIN),ig.Font.ALIGN.CENTER,undefined));options[menu.Page.QUITSURE].push(new ss.MenuOption(menu,function(){return ig.i18n().MainMenu.QuitSure;},undefined,(ig.system.width+10)/2,nextOptionSpace(menu.Page.QUITSURE),ig.Font.ALIGN.CENTER,undefined));options[menu.Page.QUITSURE].push(new ss.MenuOption(menu,function(){return ig.i18n().Answer.yes;},function(){ig.system.quit();},(ig.system.width+10)/2,nextOptionSpace(menu.Page.QUITSURE),ig.Font.ALIGN.CENTER,undefined));options[menu.Page.QUITSURE].push(new ss.MenuOption(menu,function(){return ig.i18n().Answer.no;},function(){menu.pageBack();},(ig.system.width+10)/2,nextOptionSpace(menu.Page.QUITSURE),ig.Font.ALIGN.CENTER,undefined));}
options[menu.Page.SETTINGS].push(backButton);options[menu.Page.CONTROLS].push(backButton);options[menu.Page.LOAD].push(backButton);this.options=options;},update:function()
...
{ig.system.context.fillStyle='rgb(70,70,70)';ig.system.context.fillRect(0,0,ig.system.realWidth,ig.system.realHeight);this.titleFont.draw(this.titleText(),ig.system.width/2,10,ss.TrueFont.ALIGN.CENTER,1);if(this.currentPage===this.Page.LOBBY)
...
ig.baked=true;ig.module('game.entities.void').requires('impact.entity').defines(function(){EntityVoid=ig.Entity.extend({_wmDrawBox:true,_wmBoxColor:'rgba(128, 28, 230, 0.7)',size:{x:32,y:32},update:function(){}});});
...
ig.baked=true;ig.module('plugins.ss.dialog').requires('impact.timer','impact.entity','plugins.ss.texture-canvas','plugins.ss.menu.cursor','plugins.ss.true-font').defines(function(){"use strict";if(typeof(ss)==="undefined")ss={};ss.GlobalDialogManager={dialogIsActive:false,textSize:10,minHeight:20,textLinePadding:2,textEdgePadding:2,bottomMargin:20,leftMargin:100,rightMargin:100,choiceCursorPadding:10,fontName:"sans-serif bold",fontColor:"rgba(0,0,0,1)",bgColor:"rgba(190, 190, 190, 1)",upButtons:['up'],downButtons:['down'],leftButtons:['left'],rightButtons:['right'],selectButtons:['primary'],cancelButtons:['cancel'],cursor:new ss.Cursor(),betweenActionsTimer:new ig.Timer(),trueFont:undefined,loadFont:function(name,css){this.trueFont=new ss.TrueFont(name,this.textSize,this.fontColor,css);}};var scaled=function(value){return value*ig.system.scale;}
...
var anyUpReleased=function(){for(var i=0;i<ss.GlobalDialogManager.upButtons.length;i++){if(ig.input.released(ss.GlobalDialogManager.upButtons[i])){return true;}}
return false}
var anyDownReleased=function(){for(var i=0;i<ss.GlobalDialogManager.downButtons.length;i++){if(ig.input.released(ss.GlobalDialogManager.downButtons[i])){return true;}}
return false}
var anyLeftReleased=function(){for(var i=0;i<ss.GlobalDialogManager.leftButtons.length;i++){if(ig.input.released(ss.GlobalDialogManager.leftButtons[i])){return true;}}
return false}
var anyRightReleased=function(){for(var i=0;i<ss.GlobalDialogManager.rightButtons.length;i++){if(ig.input.released(ss.GlobalDialogManager.rightButtons[i])){return true;}}
return false}
...
this.setCharacter(character);},refreshText:function()
...
{this.handler=handler;this.nextMoment=moment;},setHandler:function(handler){this.handler=handler;},setNextMoment:function(moment){this.nextMoment=moment;},trigger:async function(manager){manager.changeActiveMoment(null);if(typeof(this.handler)==="function"){await this.handler();}
...
{this.activeMoment=moment;this._prepActiveGeometry(moment);this._prepImage(moment);},_prepActiveGeometry:function(moment)
...
this.activeGeometry.textBacking.y=ss.texture2DCanvas.height-scaled(ss.GlobalDialogManager.textEdgePadding+ss.GlobalDialogManager.bottomMargin);this.activeGeometry.textBacking.unscaledY=descaled(ss.texture2DCanvas.height)-(ss.GlobalDialogManager.textEdgePadding+ss.GlobalDialogManager.bottomMargin);for(var i=0;i<this.activeGeometry.items.length;i++)
{this.activeGeometry.textBacking.y-=this.activeGeometry.items[i].height;}
var minY=ss.GlobalDialogManager.minHeight+ss.GlobalDialogManager.textEdgePadding+ss.GlobalDialogManager.bottomMargin;if(descaled(ss.texture2DCanvas.height)-this.activeGeometry.textBacking.unscaledY<minY){this.activeGeometry.textBacking.unscaledY=descaled(ss.texture2DCanvas.height)-minY;this.activeGeometry.textBacking.y=ss.texture2DCanvas.height-scaled(minY);}
var currentY=this.activeGeometry.textBacking.unscaledY+ss.GlobalDialogManager.textEdgePadding;for(var i=0;i<this.activeGeometry.items.length;i++)
{this.activeGeometry.items[i].setPosition(ss.GlobalDialogManager.textEdgePadding+ss.GlobalDialogManager.leftMargin,ss.GlobalDialogManager.textLinePadding+currentY);currentY+=this.activeGeometry.items[i].unscaledHeight;}},_choiceCursorImage:(new Image()),_textCursorImage:(new Image()),_prepCursors:function(){ss.texture2DContext.clearRect(0,0,ss.texture2DCanvas.width,ss.texture2DCanvas.height);ss.texture2DContext.font=scaled(ss.GlobalDialogManager.textSize)+"px sans-serif";ss.texture2DContext.fillStyle=ss.GlobalDialogManager.fontColor;ss.texture2DContext.fillText('▼',0,0);this._textCursorImage.src=ss.texture2DCanvas.toDataURL('image/png');ss.texture2DContext.clearRect(0,0,ss.texture2DCanvas.width,ss.texture2DCanvas.height);ss.texture2DContext.fillText('▶',0,0);this._choiceCursorImage.src=ss.texture2DCanvas.toDataURL('image/png');},_preparedImage:(new Image()),_prepImage:function(moment)
{ss.texture2DContext.clearRect(0,0,ss.texture2DCanvas.width,ss.texture2DCanvas.height);ss.texture2DContext.fillStyle=ss.GlobalDialogManager.bgColor;ss.texture2DContext.beginPath();ss.texture2DContext.rect(this.activeGeometry.textBacking.x(),this.activeGeometry.textBacking.y,this.activeGeometry.textBacking.getWidth(),this.activeGeometry.textBacking.getHeight());ss.texture2DContext.closePath();ss.texture2DContext.fill();ss.texture2DContext.fillStyle=ss.GlobalDialogManager.fontColor;ss.texture2DContext.textAlign="start";ss.texture2DContext.textBaseline="top";ss.texture2DContext.direction="ltr";ss.texture2DContext.font=scaled(ss.GlobalDialogManager.textSize)+"px "+ss.GlobalDialogManager.fontName;for(var i=0;i<this.activeGeometry.items.length;i++)
...
{var proposedIndex=this.selectedChoiceIndex-1;if(proposedIndex>=0)this.selectedChoiceIndex=proposedIndex;}
if(anyDownReleased()||anyRightReleased())
{var proposedIndex=this.selectedChoiceIndex+1;var totalItems=this.activeMoment.choices.length;if(proposedIndex<totalItems)this.selectedChoiceIndex=proposedIndex;}
...
ig.baked=true;ig.module('game.entities.ink-visual-novel').requires('impact.entity','plugins.ss.random','plugins.ss.dialog').defines(function(){EntityInkVisualNovel=ss.DialogManagedEntity.extend({_wmDrawBox:true,_wmBoxColor:'rgba(28, 230, 128, 0.7)',size:{x:32,y:32},name:"",dialogManager:undefined,init:function(x,y,settings){this.parent(x,y,settings);if(settings.cutscene!==""&&"Cutscene"+settings.cutscene in self){this.cutscene=new self["Cutscene"+settings.cutscene]();this.dialogManager=new ss.DialogManager(this.cutscene.getFirstMoment());this.dialogManager.trigger();}},update:function(){this.parent();if(this.dialogManager){this.dialogManager.update();}},draw:function(){this.parent();if(this.dialogManager){this.dialogManager.draw();}},beingTriggered:false,trigger:function()
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('plugins.impact-splash-loader','impact.game','impact.font','plugins.ss.true-font','plugins.chrome-app','plugins.localstorage','game.colors','game.logos','game.menu.title','game.entities.void','game.entities.ink-visual-novel','plugins.ss.dialog','game.levels.blank').defines(function(){ig.Sound.use=[ig.Sound.FORMAT.OGG,ig.Sound.FORMAT.M4A]
VNGGJ2018=ig.Game.extend({musicEnabled:true,localStorage:new ig.localStorage(),menu:null,logos:new Logos(),fadeInTime:1,fadeOutTime:1,fadeInTimer:undefined,fadeOutTimer:undefined,titleMusic:new ig.Sound('media/music/Kevin_MacLeod_-_Carefree.*',false),init:function(){ig.music.add(this.titleMusic,"Title");ig.music.volume=0.4;ig.music.play("Title");ig.game.localStorage.get("music-enabled",function(musicEnabled){if(musicEnabled===null)return;console.debug("Setting loaded: music-enabled = "+musicEnabled);ig.game.musicEnabled=typeof(musicEnabled)==="boolean"?musicEnabled:musicEnabled==="true";if(ig.game.musicEnabled)ig.music.play();else ig.music.stop();});this.setupDialogManager();ig.input.bind(ig.KEY.MOUSE1,'click');if(ig.ua.mobile){}else{this.setupDesktopControls();}
this.fadeInTimer=new ig.Timer(0);this.fadeOutTimer=new ig.Timer(0);this.logos.run(function(){ig.game.fadeInTimer=new ig.Timer(ig.game.fadeInTime);ig.game.loadMainMenu();},true);},setupDialogManager:function(){ss.GlobalDialogManager.fontName="Cavalcade";setTimeout(function(){ss.GlobalDialogManager.loadFont("Cavalcade","/fonts/Cavalcade/Cavalcade-Regular.css");},1);ss.GlobalDialogManager.textSize=36;ss.GlobalDialogManager.textEdgePadding=10;ss.GlobalDialogManager.textLinePadding=10;ss.GlobalDialogManager.bgColor=hexToRgbA(GameColors.DialogBG);ss.GlobalDialogManager.fontColor=hexToRgbA(GameColors.DialogFG);ss.GlobalDialogManager.minHeight=100;},loadMainMenu:function()
{this.menu=new TitleMenu();ig.music.play("Title");},setupDesktopControls:function(){ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.SPACE,'primary');ig.input.bind(ig.KEY.BACKSPACE,'secondary');ig.input.bind(ig.KEY.ENTER,'pause');ss.GlobalDialogManager.selectButtons.push('primary');ss.GlobalDialogManager.cancelButtons.push('secondary');if(ig.system.isNMJSEnvironment)
...
{ig.game.autoSort=true;ig.game.sortBy=ig.Game.SORT.POS_Y;this.fadeOutTimer=new ig.Timer(this.fadeOutTime);setTimeout(function(){ig.music.fadeOut();if(location.search.length>0)ig.game.loadLevel(self["Level"+location.search.replace("?level=","")]);else ig.game.loadLevel(LevelBlank);ig.game.fadeInTimer=new ig.Timer(ig.game.fadeInTime);},ig.game.fadeOutTime*1000);},loadLevel:function(data)
...
if(this.fadeOutTimer.delta()<0){var fadeAlpha=(ig.game.fadeOutTime+this.fadeOutTimer.delta())/ig.game.fadeOutTime;ig.system.context.fillStyle='rgba(0,0,0,'+fadeAlpha+')';ig.system.context.fillRect(0,0,ig.system.realWidth,ig.system.realHeight);}},toggleMusic:function()
{if(ig.game.musicEnabled)ig.game.stopMusic();else ig.game.startMusic();return ig.game.musicEnabled;},startMusic:function()
{if(ig.game.musicEnabled==false)
{ig.game.musicEnabled=true;ig.music.play();ig.game.localStorage.set("music-enabled",true);}
return ig.game.musicEnabled;},stopMusic:function()
{if(ig.game.musicEnabled==true)
{ig.game.musicEnabled=false;ig.music.stop();ig.game.localStorage.set("music-enabled",false);}
return ig.game.musicEnabled;},});var fps=60;var gameWidth=720;var gameHeight=gameWidth*(16/9);var scale=1;ig.System.scaleMode=ig.System.SCALE.SMOOTH;ig.main('#canvas',VNGGJ2018,fps,gameHeight,gameWidth,scale,ig.ImpactSplashLoader);});
