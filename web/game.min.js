...
{this.handler=handler;this.nextMoment=moment;},setHandler:function(handler){this.handler=handler;},setNextMoment:function(moment){this.nextMoment=moment;},trigger:function(manager){var choice=manager.selectedChoiceIndex;manager.changeActiveMoment(null);if(typeof(this.handler)==="function"){this.handler(choice);}
...
ig.music.volume=1;ig.music.loop=true;this.setupDialogManager();ig.input.bind(ig.KEY.MOUSE1,'click');this.setupDesktopControls();this.fadeInTimer=new ig.Timer(0);this.fadeOutTimer=new ig.Timer(0);this.logos.run(function(){ig.game.fadeInTimer=new ig.Timer(ig.game.fadeInTime);ig.game.loadMainMenu();setTimeout(function(){if(!ig.ua.mobile)ig.music.play("menu");},400);},true);},setupDialogManager:function(){ss.GlobalDialogManager.fontName="Cavalcade";setTimeout(function(){ss.GlobalDialogManager.loadFont("Cavalcade","/fonts/Cavalcade/Cavalcade-Regular.css");},1);ss.GlobalDialogManager.textSize=36;ss.GlobalDialogManager.textEdgePadding=10;ss.GlobalDialogManager.textLinePadding=10;ss.GlobalDialogManager.outlineThickness=2;ss.GlobalDialogManager.choiceCursorPadding=30;ss.GlobalDialogManager.outlineColor=hexToRgbA(GameColors.DialogBGOutline);ss.GlobalDialogManager.bgColor=hexToRgbA(GameColors.DialogBG);ss.GlobalDialogManager.fontColor=hexToRgbA(GameColors.DialogFG);ss.GlobalDialogManager.minHeight=1000;},loadMainMenu:function()
...
if(this.fadeOutTimer.delta()<0){var fadeAlpha=(ig.game.fadeOutTime+this.fadeOutTimer.delta())/ig.game.fadeOutTime;ig.system.context.fillStyle='rgba(0,0,0,'+fadeAlpha+')';ig.system.context.fillRect(0,0,ig.system.realWidth,ig.system.realHeight);}}});var fps=60;var gameWidth=720;var gameHeight=gameWidth*(16/9);var scale=1;ig.Sound.use=[ig.Sound.FORMAT.OGG,ig.Sound.FORMAT.M4A];ig.System.scaleMode=ig.System.SCALE.SMOOTH;ig.main('#canvas',SwitchboardCopperGame,fps,gameHeight,gameWidth,scale,ig.ImpactSplashLoader);if(ig.ua.mobile)
{var fixMobileAudio=function(event)
{for(var i=0;i<ig.music.tracks.length;i++)
{ig.music.tracks[i].play(event);ig.music.tracks[i].pause();}
ig.music.play("menu");document.getElementById("mobile-play").removeEventListener("click",fixMobileAudio);document.getElementById("mobile-play").style.display='none';}
document.getElementById("mobile-play").addEventListener("click",fixMobileAudio);}else{document.getElementById("mobile-play").style.display='none';}});
