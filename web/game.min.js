...
ig.baked=true;ig.module('game.menu.title').requires('game.i18n','plugins.ss.menu.menu','plugins.ss.true-font').defines(function(){TitleMenu=ss.Menu.extend({startBlinkTimer:null,useTrueFont:true,trueFont:new ss.TrueFont("Cavalcade",42,"#666","/fonts/Cavalcade/Cavalcade-Regular.css"),useSelectedFont:true,selectedTrueFont:new ss.TrueFont("Cavalcade",42,"#333","/fonts/Cavalcade/Cavalcade-Regular.css"),Page:{MAIN:0,SETTINGS:1,CONTROLS:2,LANGUAGE:3,LOAD:4,QUITSURE:5},actions:{prev:["up","left"],next:["down","right"],select:["primary"],back:["secondary"]},init:function()
{this.startBlinkTimer=new ig.Timer();this.createMenuOptions();this.parent();},titleFont:new ss.TrueFont("Cavalcade",64,"#444","/fonts/Cavalcade/Cavalcade-Regular.css"),titleText:function(){return ig.i18n().GameTitle;},pageBack:function()
...
{ig.system.context.fillStyle='#f7efd7';ig.system.context.fillRect(0,0,ig.system.realWidth,ig.system.realHeight);this.titleFont.draw(this.titleText(),ig.system.width/2,10,ss.TrueFont.ALIGN.CENTER,1);if(this.currentPage===this.Page.LOBBY)
...
this.setNextMoment(inkstory.getNextMoment(choiceIndex));});this.ink.tagHandlers["portrait"]=function(key,character,position){if(!position)position="left";if(character==null||character=="none"){inkstory.currentPortrait=undefined;}else{inkstory.currentPortrait={character:character,position:position};}};this.ink.tagHandlers["background"]=function(key,name){inkstory.currentBackground=name==null||name=="none"?undefined:name;};this.ink.tagHandlers["music"]=function(key,title){if(title in ig.music.namedTracks){console.debug("🎵 "+title);}else{console.debug("🎵 ??"+title);return;}
if(ig.music.currentTrack!==ig.music.namedTracks[title])ig.music.play(title);};},setDialogManager:function(manager){this.dialogManager=manager;},getNextMoment:function(choiceIndex){this.ink.continue(choiceIndex);if(typeof(this.currentPortrait)==="undefined"){console.log("☺️ -");}else{console.log("☺️ "+this.currentPortrait.character+" - "+this.currentPortrait.position);}
var text=this.ink.getText();var sizedText="";var needNewline=false;for(var i=0;i<text.length;i++){sizedText+=text[i];if(i>64&&i%65===0){needNewline=true;}
if(needNewline){if(/[\s\.?!\-+=]/.test(text[i])){sizedText+='\n';needNewline=false;}else if(i!==text.length-1&&i>71&&i%72===0){sizedText+='-\n';needNewline=false;}}}
...
VNGGJ2018=ig.Game.extend({musicEnabled:true,localStorage:new ig.localStorage(),menu:null,logos:new Logos(),fadeInTime:1,fadeOutTime:1,fadeInTimer:undefined,fadeOutTimer:undefined,music:{menu:new ig.Sound('media/music/Jahzzar - Airship Fury.*',false),hall:new ig.Sound('media/music/Kevin MacLeod - Hall of the Mountain King.*',false),hall1:new ig.Sound('media/music/Kevin MacLeod - Hall of the Mountain King - 1.*',false),hall2:new ig.Sound('media/music/Kevin MacLeod - Hall of the Mountain King - 2.*',false),hall3:new ig.Sound('media/music/Kevin MacLeod - Hall of the Mountain King - 3.*',false),hall4:new ig.Sound('media/music/Kevin MacLeod - Hall of the Mountain King - 4.*',false),hall5:new ig.Sound('media/music/Kevin MacLeod - Hall of the Mountain King - 5.*',false)},init:function(){for(var i in this.music){ig.music.add(this.music[i],i);}
